...
5. **Image Classification** — a "coming soon" placeholder for now (see
   below). The tabular pages are fully working.

On first load, a short 5-step onboarding tour walks through what each tab
does before you land in the app — "Skip tour" jumps straight in, and it can
be replayed anytime from the sidebar.

## Image Classification — coming soon

This tab is currently a placeholder describing what's planned: uploading a
zip of labeled image folders, fine-tuning a pretrained image model via
transfer learning, a live training curve, testing on a new image, and
downloading the trained weights. Deep learning support (torch/torchvision)
isn't wired up yet — it's a deliberately separate future addition so it
doesn't add a heavy dependency to the working tabular app in the meantime.
...
- No team/auth system — everything lives in Streamlit's `session_state`
  ...
  and restarts the onboarding tour.
- Tabular data only for now — image classification is planned but not yet built.
